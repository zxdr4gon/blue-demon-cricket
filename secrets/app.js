/* =========================================================
   BLUE DEMON CRICKET — Multi-season live data
   ========================================================= */
const SHEET_CONFIG = {
  "2026-27": {
  ...
  return rows
    ...
    .map(row => {
      ...
      columns.forEach((column, index) => {
        out[column] = String(row[index] ?? "").trim();
      });
      return out;
    })
...
async function fetchCsv(url) {
  ...
  // Google Sheets may publish an unused blank column before the real headers.
  // Remove it before mapping the sheet columns to the app's expected columns.
  if (String(rows[0]?.[0] ?? "").replace(/^\uFEFF/, "").trim() === "") {
...
function openPlayerModal(name) {
  ...
  hero.insertAdjacentHTML("beforeend", `
    <div>
      <p class="eyebrow">${escapeHtml(APP.season)} PLAYER PROFILE</p>
